 ...
 * @brief Hook script that runs after the config plugin is installed on iOS.
 *
 * Updates the FirebaseRemoteConfig pod version in the Podfile based on the
...
var PLUGIN_ID = "cordova-plugin-firebasex-config";
...
function resolvePluginVariables(context) {
    ...
    try {
        ...
        if (fs.existsSync(configXmlPath)) {
            ...
            console.warn("[FirebasexConfig] config.xml not found at " + configXmlPath + ". Cannot read plugin variables from config.xml.");
        }
    } catch (e) {
        console.warn("[FirebasexConfig] Could not read config.xml for plugin variables: " + e.message);
    }
    ...
    try {
        ...
        if (fs.existsSync(packageJsonPath)) {
            ...
            console.warn("[FirebasexConfig] package.json not found at " + packageJsonPath + ". Cannot read plugin variables from package.json.");
        }
    } catch (e) {
        console.warn("[FirebasexConfig] Could not read package.json for plugin variables: " + e.message);
    }
 ...
 * Updates the FirebaseRemoteConfig pod version in the Podfile.
 *
 * @param {object} context - The Cordova hook context.
 */
module.exports = function(context) {
    ...
    if (!pluginVariables["IOS_FIREBASE_SDK_VERSION"]){
        console.warn("[FirebasexConfig] IOS_FIREBASE_SDK_VERSION variable not set. Skipping Podfile update for FirebaseRemoteConfig pod version.");
        return;
    }
    ...
    if (!fs.existsSync(podFilePath)) {
        console.warn("[FirebasexConfig] Podfile not found at " + podFilePath + ". Cannot update FirebaseRemoteConfig pod version.");
        return;
    }
    ...
    try {
        ...
        var podRegEx = /pod 'FirebaseRemoteConfig', '(\d+\.\d+\.\d+[^'"]*)'/g;
        ...
        if (matches) {
            ...
            if (modified) {
                ...
                console.log("[FirebasexConfig] Firebase SDK version set to v" + sdkVersion + " in Podfile");
            }
        }
    } catch(e) {
        console.warn("[FirebasexConfig] Error updating Firebase RemoteConfig pod version: " + e.message);
    }
};
